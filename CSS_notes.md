...
:star::star::star::star::star:再次强调，这个优先级是不同情况下 修饰相同样式的情况下的比较，如果不是 相同样式，还是叠加的，比如，都是修饰 color：red的话，才会比较然后覆盖，如果一个是font-size，一个是border是互不影响的
...
1. position属性的取值+ left/right/top/bottom配合使用
   ...
   - relative
     ...
     - left/right/top/bottom是相对于是离你最近的relative的父元素的整体位置发生偏移

       >  	1. 比如你父元素relative是一个200\*300的div，那么你right：30px，意思就是这个200\*300的框框的右边 塞了30px，和 left=-30px是一个意思。
   ...
   - absolute
     ...
     - left/right/top/bottom是相对于是离你最近的relative的父容器的**左上角**为启示

       > 	1. ***注意他此时已经脱离文档流了，除了他以外，父类里面还有什么已经不影响了***
       >  	2. absolute的上面的父元素必有 relative/absolute/fixed中的一种，否则无法定位。
       >  	3. 比如你父元素relative是一个200\*300的div，那么你right：30px，意思就是你的absolute的子容器会往左走30px。
       >  	4. 如果全部没有，那就会按照这个html来当做父元素，一般不会发生，除非你是傻逼。

     -  **为什么一般写的值 都是top和 left，而不是right！？**

       > ​	因为如上所说，他的起始点是**左上角**，那如果你写一个bottom或者right的话，他根本没地儿去，因为已经顶格了，此时呢，他就会 绕一圈，从另一头出来，比如你开头就写right=10px的话，因为你已经在最最最左边了，右边塞10个px的话，会顶出父容器，所以呢，你会从最右边滚出来，10个px。所以，综上所述，平时position absolute位置的时候，只写top和left就行。
       >
       > 如果你是配合伪类before+a标签这种，必须左边溢出父容器的话，你就写 负的left。千万别写right。不是等价的。
   
     - 如果你想absolute的A元素的子元素B也想定位absolute怎么办?

       > ​	此时我们需要换个角度，把B提出来，和A并列为兄弟，然后同时向relative的父元素做absolute，因为 后absolute的也就是后脱离文档流的，会覆盖在前absolute的那个元素上，会盖住，所以我们可以让 按钮的absolute在后，图片的absolute在前，这样就可以达到图片上有按钮的效果，并同时完成绝对定位！
...
- sticky
  ...
  - 偏移量相当于 该元素和当前窗口的位置关系，比如top:100px,代表该元素和当前该窗口的上部相距离100px
    ...
    > ​	一般来说，用途就是 页面下拉到一定高度就sticky在页面的头部的 导航栏。
...
- index
     ...
     - 需要注意的是：如果出现这种情况： 
       ...
       //如果父元素不设置任何z-index，那么谁的index大谁就在上面，只能同级比较。
...
> ​	另外有一种特殊情况，就是你的span啊p啊这些标签已经变成block了，不是行内元素了，那么要把text-align加在p或者span上，
>
> 总而言之，就是这个textalign属性必须加在 最父亲的第一个非行内元素的容器属性里面。

----

## a标签覆盖原容器

1. a标签覆盖原容器 使得其容器可以 点击全部尺寸都可以点出链接。

2. 为了seo，我们需要把a标签写文字，但是需要隐藏
...
1. 首先，我们知道：
   ...
   //div.box的优先级更高，会覆盖掉.box的样式，虽然两个修饰的都是同一个地方的字体颜色。
   ```

   

2. 但是要注意的是，在嵌套结构中，也会出现优先级的问题，比如，上面的例子，如果div1嵌套了一个span， div1的css样式是字体为红，这个span我要把他设置成字体为蓝。

   这个时候，我如果把div1选择器设置成 id+class， 而把div2选择器写成 div1加span标签： 
   ...
   //此时，虽然span是嵌套在里面的，而且也特意位置了color为blue，但是不会起作用，因为 .div1 span{}的写法，优先级没有 .div1 #div1的优先级高，会导致在渲染字体方面，还是红色。这是 在嵌套结构中容易出现的优先级错误。
   ```

   

   3. > ​	所以，说白了，最好的解决办法，就是，在写选择器的时候，如果没有id，在写class时候，尽量层级详尽一点。 你要知道，2个class相加的优先级， 就比 class+p/span优先级高了。
...
1. 直接一个给这个a标签，写一个 before伪类，或者after伪类
2. 里面 content不写， 转成**inline-block**，设置宽高，和背景图片，就是你的图标
3. 然后重点来了。在before里定位position abosulet， 在a标签定位relative，或者a标签的父类无所谓。
4. 然后微调位置，把图标拜访在a标签之前
5. 这是主流的做法。

处理方法2：

1. 设置一个div，里面是a标签，但是div的宽度比a要大。
2. a标签的左边padding-left，值就是图片的大小。
3. 然后设置div的背景图片，position设置为left再加一个偏移量，确定好图标所在位置。
4. 说白了，最后的原理就是 这个div的长度= a的长度+a的padding left，但是a又没有背景，所以显示出来的就是div的背景，那div的背景又是【图标+空白】，所以就可以实现图标+a。

处理方法3：这种方法好，可以复用

1. 在·a标签后面写一个i标签，给i标签一个class
2. 然后给i转成block或者inline-block，然后设置图标为背景，并设置宽高
3. 然后用position absolute微调。
...
> 1. 他的定位是相对于 最近的一个relative，一般来说就是其父元素的左上角来定位的。
>
> 2. **特别重要：**但是它相当于的这个定位，在调整top和left的值的时候要注意。
>
>    > ​	**它定位超过不了父容器！！！，比如你div 300乘300，你父容器400*400，你不能top 大于100，这样下面会消失！！虽然他脱离文档流，但是跑不出父容器的范围！而且因为脱离文档流，overflow：hidden也没用！！**平时一般不会遇到这种情况，因为你大布局用div分割好了，小布局的absolute肯定不会跑太远，所以超不过relative的父容器，所以这点容易忽视。
>
> 3. **高度塌陷问题：**！？ 最简单的方法避免高度塌陷问题就是！
>
>    就是说因为你父元素没用设置匡高，然后又脱离文档流，导致父元素直接没用内容了，撑不起来宽，就会导致比如背景图消失的bug。
>
>    > ​	做relative的父元素一定要设置宽高， 并且一定要比 absolute的div匡高要大！保证没问题！
>    >
>    >    
>
> 4. aboslute之后后面的div会顶上去：
>
>    > ​	如果他下面还有元素，那么因为**脱离文档流**。下面的元素会顶上来，那么避免这个问题，你需要加一个相同宽高的空白div。
