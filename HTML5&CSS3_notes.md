...
2. 一个网页 不是说永远只有一种做法（弹性、rem、定位%）任选一个，而是分段处理的， 相当于 小尺寸上一个布局，中尺寸上一个，大尺寸上一个，每个尺寸下都有各自的方法，也就是说 比如 同一个导航栏，在大尺寸上可能是 什么也不做，只用定位+固定px，但是中尺寸下就变成了 rem自适应，小尺寸下又变成了弹性布局。这都是很有可能的。

3.  每一个分段下，其实也不是说 不是说永远只有一种做法（弹性、rem、定位%）任选一个，而是混杂在一起的。比如 有三个div，一个导航栏，一个main，一个footer，很有可能他们是用了不同的处理方式，比如 在nav，他用固定px定位%，在mian他用rem，在footer或者article他用flex。
...
5. 作用域
   ...
   > 1. 在less里，变量才用就近原则，上下都会受到影响，更近的@number就会被用上，比如选择器内部的。
   > 2. **但是在sass里，变量是有顺序的，$char只能影响下方，而$chara不能影响所在上方。**
